...
def solveOT(ns, nt, S_, h_, X_):
    ...
    # sử dụng simplex method.
    # nếu lỗi chỗ này (basis variable) thì đọc phần Note files to replace Repo: SI-SeqFS-DA
    # Solve wasserstein distance
    res = linprog(
        cost,
        A_ub=-np.identity(ns * nt),
        b_ub=np.zeros((ns * nt, 1)),
        A_eq=S_,
        b_eq=h_,
        method="simplex",
